...
/mob/living/exosuit/premade/cubical
	name = "Tracked exosuit"
	desc = "A heavily armored, tracked exosuit."

/mob/living/exosuit/premade/cubical/Initialize()
	if(!arms)
		arms = new /obj/item/mech_component/manipulators/heavy(src)
		arms.color = COLOR_TITANIUM
	if(!legs)
		...
		legs.color = COLOR_TITANIUM
	if(!body)
		body = new /obj/item/mech_component/chassis/cubical(src)
		body.color = COLOR_TITANIUM

	. = ..()

/mob/living/exosuit/premade/cubical/spawn_mech_equipment()
	..()
	install_system(new /obj/item/mech_equipment/mounted_system/taser/laser(src), HARDPOINT_LEFT_HAND)
	install_system(new /obj/item/mech_equipment/mounted_system/taser/plasmagun(src), HARDPOINT_RIGHT_HAND)
	install_system(new /obj/item/mech_equipment/shields(src), HARDPOINT_BACK)
